...
Sidebar::Sidebar(float left, float width):_left(left), _width(width){
    ...
    items.push_back("------------Side Bar------------");
    //Fill the items vector with empty strings so that we can use [] to read them:
    for (int i=0 ; i<40; i++){
        items.push_back("");
    }

    items[SB_EQUATION]      = "Equations: ";
    items[SB_INSTRUCTION_1] = "[I]: Zoom in    [O]: Zoom out";
    items[SB_INSTRUCTION_2] = "[R]: Reset equation history";
    items[SB_INSTRUCTION_3] = "[S]: Save       [L]: Load";
    items[SB_INSTRUCTION_4] = "[Arrow keys]: Pan";
    items[SB_INSTRUCTION_5] = "[F]: Input a new equation";
    

    //get a vector of position from sb_text
    ...
    for (vector<string>::iterator it = items.begin();
        ...
        pList.push_back(sb_text.getPosition());
...
vector<sf::Vector2f> Sidebar::position(){
    return pList;
}
